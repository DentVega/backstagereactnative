...
## 2026-07-21 (cont.) — Botón "Deploy" (1 click → CI build+publish) + fixes de CI
- **Raw request:** el usuario notó que no había botón para deployar en la UI de Backstage ni estado de CI real. Se atacó el botón Deploy (opción A).
- **Feature (backstage-web):** botón **"Deploy"** en la página de detalle (para logins allowlisted) → `POST /api/miniapps/:id/deploy` (auth por sesión) → parsea owner/repo del `repoUrl` del registry → `GitProvider.dispatchWorkflow` hace `POST /repos/{o}/{r}/actions/workflows/ci.yml/dispatches`. 131 tests. El scope `repo` del token alcanza para disparar (204 verificado). Commit `feat(deploy): one-click Deploy button`.
- **Miniapp CI:** `workflow_dispatch` agregado al `ci.yml` de account-dashboard + template.
- **DEBUG del CI (2 bugs reales encontrados corriendo el CI):**
  1. **Install 401:** pnpm v10 **ignora** el `${GITHUB_TOKEN}` en un `.npmrc` **committeado** (política de seguridad) → sin auth → 401 en `@dentvega/*`. Además el token automático de Actions no alcanzaba paquetes user-scoped *restricted*. **Fix:** (a) `@dentvega/miniapp-contract` + `@dentvega/ui-kit` puestos **públicos** (el token automático ya lee públicos); (b) el workflow escribe el token a `~/.npmrc` (user-level, que pnpm sí acepta) antes del install.
  2. **Sub-chunks en path equivocado:** el CI zippeaba `build/` entero → chunks anidados bajo `generated/android/` → el host los busca junto al container → 404 → la miniapp NO montaba (container 200 pero sub-chunks 404). **Fix:** `cd build/generated/android && zip -r $GITHUB_WORKSPACE/build.zip .` (contenido plano).
- **Secrets (usuario):** `BACKSTAGE_URL` + `PUBLISH_TOKEN` como secrets del repo de la miniapp (por-repo, porque DentVega es usuario, no org).
- **Verificado end-to-end:** run `push` de 0.7.0 = **success**; 0.7.0 en el catálogo (count 6); container + sub-chunks servidos en la raíz de la versión (200); **0.7.0 (chunk del CI) monta en el emulador** con datos + tema. El `useTheme` que apareció era Metro con bundle viejo pre-rename (restart lo arregló; no es bug del CI).
- **Apuntes abiertos:** versión estática en `manifest.json` → repetir versión da "blob already exists" (a futuro auto-bump/allowOverwrite); secrets por-repo (automatizar en el scaffolder); paquetes `@dentvega/*` ahora PÚBLICOS (los repos de miniapp siguen privados). Estado de CI real en la UI (badge) sigue en "unknown" (scope `read:user`) — no atacado.
