...
- **Activated:** **build DESBLOQUEADO (2026-07-13)** — `./gradlew assembleDebug` compila ✅ (`app-debug.apk`, 127 MB). Falta solo instalar en dispositivo/emulador (device no conectado ahora).

## ✅ BLOQUEANTE RESUELTO (2026-07-13) — era la distro del JDK (Zulu 17)
**Root cause:** el `MissingValueException` en el grafo completo de `assembleDebug` era específico de **Azul Zulu 17.0.13**, la única JVM instalada. Confirma la teoría "es el ENTORNO, no el proyecto".
**FIX:** `brew install openjdk@17` (Homebrew OpenJDK **17.0.19**, sin sudo — el cask de Temurin requería sudo interactivo y falló). Build con
`JAVA_HOME=/opt/homebrew/opt/openjdk@17/libexec/openjdk.jdk/Contents/Home` →
**`BUILD SUCCESSFUL in 28s`** (217 tareas). Todos los fixes previos (gradle-plugin devDeps, NDK reparado, substitution react-android/hermes-android, screens 4.11.1) se conservaron; solo faltaba cambiar la JVM.
**Durabilidad:** apuntar `JAVA_HOME` a openjdk@17 (o `org.gradle.java.home` en `~/.gradle/gradle.properties`) para que futuros builds no vuelvan a Zulu.

## Native (Re.Pack app) — PROGRESO 2026-07-09 (histórico; ya desbloqueado arriba)
